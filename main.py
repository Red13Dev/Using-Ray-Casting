...
# Map generator
size = 15
map1 = [[list(np.random.uniform(0, 1, 3))] * size for i in range((size))]
...
while True:
    ...
    if testx > 0 and testx < size - 1 and testy > 0 and testy < size - 1:
        if map1[testx][testy] == 0 or count > 5:
            count = 0
            x, y = (testx, testy)
            map1[x][y] = 0
            if x == size-2:
        ...
        while True:
            ...
            if abs(int(3*xx)-int(3*x)) > 0 or abs(int(3*yy)-int(3*y)) > 0:
                ...
                if int(x) == exitx and int(y) == exity:
                    ...
                    tilec.append("k")
            if map1[int(x)][int(y)] != 0:
                h = np.clip(1/(0.02 * n), 0, 1)
                c = np.asarray(map1[int(x)][int(y)]) * (0.3 + 0.7 * h**2)
                break
        plt.vlines(i, -h, h, lw=8, colors=c)

    # Plot creation and updating
